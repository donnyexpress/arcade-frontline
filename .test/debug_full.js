...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    state.sides.blue.buildings.push({
      type: 'barracks', side: 'blue', x: 400, y: 300,
      hp: 80, maxHp: 80, dmg: 0, range: 0, constructing: false
    });
    state.units.push({
      id: 'test_r1', type: 'rifleman', side: 'red',
      x: 100, y: 300, hp: 20, maxHp: 20, dmg: 4, range: 60, speed: 200,
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const result = await page.evaluate(() => {
      ...
      return {
        rifleman: red ? {x: Math.round(red.x), hp: red.hp, cooldown: red.attackCooldown.toFixed(2)} : 'gone',
    ...
    if (result.matchOver || result.rifleman === 'gone') {
      console.log('END at T=' + ((i+1)*0.5) + 's');
      break;
    }
  }
  await page.screenshot({ path: '/workspace/.test/v41_full.png' });
